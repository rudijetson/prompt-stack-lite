...
var init_src = __esm({
  "../packages/env/src/index.js"() {
    ...
    PROMPT_STACK_HOME = import_path.default.join(import_os.default.homedir(), ".prompt-stack");
    PATHS = {
      // Root
      home: PROMPT_STACK_HOME,
      // Installed packages - shared with Studio for unified discovery
      packages: import_path.default.join(PROMPT_STACK_HOME, "packages"),
      stacks: import_path.default.join(PROMPT_STACK_HOME, "stacks"),
      // Shared with Studio
      ...
      bins: import_path.default.join(PROMPT_STACK_HOME, "bins"),
      // Lockfiles
      ...
      // Config
      config: import_path.default.join(PROMPT_STACK_HOME, "config.json"),
      // Logs
...
async function writeLockfile(resolved) {
  ...
  const lockDir = import_path3.default.join(PATHS.locks, kind + "s");
...
async function installSinglePackage(pkg, options = {}) {
      ...
      try {
        ...
        import_fs4.default.writeFileSync(
          ...
          JSON.stringify({
            ...
            installedAt: (/* @__PURE__ */ new Date()).toISOString(),
            source: "pip",
            venvPath: import_path4.default.join(installPath, "venv")
          }, null, 2)
        );
        return { success: true, id: pkg.id, path: installPath };
      } catch (error) {
  ...
  if (import_fs4.default.existsSync(installPath)) {
    import_fs4.default.rmSync(installPath, { recursive: true });
  }
  import_fs4.default.mkdirSync(installPath, { recursive: true });
  const manifest = {
    id: pkg.id,
...
async function uninstallPackage(id) {
  ...
  try {
    import_fs4.default.rmSync(installPath, { recursive: true });
    const [kind, name] = parsePackageId(id);
    const lockPath = import_path4.default.join(PATHS.locks, kind + "s", `${name}.lock.yaml`);
...
var SECRETS_PATH = import_path5.default.join(import_os2.default.homedir(), ".prompt-stack", "secrets.json");
...
async function runStack(id, options = {}) {
  ...
  const proc = (0, import_child_process.spawn)(command, [entryPath], {
    cwd: cwd || packagePath,
    env: runEnv,
